// task1: Initialize 'transactions' as an empty array
let transactions = [];
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    // task2: Retrieve values from form inputs
    ...
    if (isEditing) {
      ...
      document.querySelector('button[type="submit"]').textContent =
        "Add Transaction";
      await updateTotalTransactionAmount(); // async usage
    } else {
...
function addTransaction(payee, amount, reason) {
  ...
  // task3: Populate newTransaction with actual values
  ...
  // task4: Push to transactions array
...
function deleteTransaction(id) {
  ...
  if (index !== -1) {
    // task6: Remove transaction using splice
...
// task7: Convert this function to async
